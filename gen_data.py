...
c_root = os.getcwd()+os.sep
dev = open(os.path.join(c_root+"data", "example.dev"),'w',encoding='utf8')
train = open(os.path.join(c_root+"data", "example.train"),'w',encoding='utf8')
test = open(os.path.join(c_root+"data", "example.test"),'w',encoding='utf8')
biaoji = set(['HOUS', 'LOC'])
fuhao = set(['。','?','？','!','！'])
dics = csv.reader(open("des_location.csv",'r',encoding='utf8'))
for row in dics:
    if len(row) == 2:
...
split_num = 0
fp = open(c_root+"datasource.txt",'r',encoding='utf8')
for line in fp:
    split_num += 1
    words = pseg.cut(line)
